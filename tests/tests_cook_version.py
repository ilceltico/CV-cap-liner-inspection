...
def test():
    ...
    prefixed = [file for file in os.listdir('./caps') if file.startswith("g_06")]
    for file in prefixed:
        print("--------------------------------------------------------------------")
        print(file)
        img = cv2.imread('caps/' + file, cv2.IMREAD_GRAYSCALE)
        ...
        blobX = [element for blob in blobs for element in blob[0]]
        blobY = [element for blob in blobs for element in blob[1]]
        ...
        x, y, rCap, cooks_distances = circledetection_cook_version.leastSquaresCookVersion(blobX, blobY)
        
        # t2 = cv2.getTickCount()
        # time = (t2 - t1)/ cv2.getTickFrequency()

        # print("Cook time: " + str(time))

        # t1 = cv2.getTickCount()
        # x, y, r = circledetection.leastSquaresCircleFitCached(blobX, blobY)
        # t2 = cv2.getTickCount()
        # time = (t2 - t1)/ cv2.getTickFrequency()
        
        # print("Time: " + str(time))

        # print("Cook: Center: (" + str(xCook) + "," + str(yCook) + "), radius: " + str(rCook))
        # print("Center: (" + str(x) + "," + str(y) + "), radius: " + str(r))
        ...
        mask = linerdefects_gradient.circularmask(img.shape[0], img.shape[1], (y, x), 0.99*rCap)
        edges[~mask] = 0
        ...
        x, y, r, cooks_distances = circledetection_cook_version.leastSquaresCookVersion(blobX, blobY)

        normalizedCooks = np.array(cooks_distances)/max(cooks_distances) * 200 + 55

        imgCook = cv2.cvtColor(img, cv2.COLOR_GRAY2BGR)
        for i in range(0, len(blobX)):
            imgCook[blobX[i],blobY[i]] = (normalizedCooks[i],100,0)
