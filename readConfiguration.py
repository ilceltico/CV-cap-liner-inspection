...
def parse_json():
    ...
    with open('config.json') as configFile:
        ...
        if INNER_METHOD == 'hough':
            ...
            if not (isinstance(INNER_LEAST_SQUARES_SPLIT, bool)) or not (INNER_LEAST_SQUARES_OUTLIERS_TYPE in ["mean", "bin"]) or not (INNER_LEAST_SQUARES_CIRCLE_GENERATION in ["mean", "interpolation", "interpolation_cook"]):
                print("Configuration error. See README.md to configure the software properly.")
                #return None
                raise SystemExit(0)
